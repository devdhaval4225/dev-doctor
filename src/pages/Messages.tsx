...
const Messages = () => {
  ...
  return (
    <div className="h-[calc(100vh-64px)] md:h-screen p-4 md:p-6 flex gap-6">
      {/* Chat List (Sidebar style) */}
      <div className="w-80 bg-white rounded-2xl shadow-sm border border-gray-100 hidden md:flex flex-col">
      ...
      <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-white">
           <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-gray-800">Louis Litt</h3>
          ...
          {messages.map((msg) => (
            <div key={msg.messageId} className={`flex ${msg.sender === 'doctor' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] rounded-2xl px-4 py-3 ${
                ...
                <p className={`text-[10px] mt-1 text-right ${msg.sender === 'doctor' ? 'text-blue-100' : 'text-gray-400'}`}>
                  {msg.timestamp}
                </p>
              </div>
            </div>
        ...
        {/* Input Area */}
        <form onSubmit={handleSend} className="p-4 bg-white border-t border-gray-100 flex items-center gap-4">
          ...
          <input 
            ...
            className="flex-1 bg-gray-50 text-gray-900 px-4 py-3 rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
          <button 
            type="submit"
            className="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 transition-colors shadow-md disabled:opacity-50"
